...
export const ZMQ_EXPERIMENT = process.env.ZMQ_EXPERIMENT ? process.env.ZMQ_EXPERIMENT : "userdiff_manual";

class ZMQWrapper {
    sock?: zmq.Request;
    sessionCount: number = 0;

    state: "session_request" | "sessions_request" | "session_unlock" | "none" = "none";
     ...
     * @returns 
     */
    async getSession(site?: string, accountId?: number) {
        if (!this.sock) {
    ...
    async getSessions(site?: string, count = 2) {
        if (!this.sock) {
            Logging.error(`ZMQ-Socket not initialized during call to get a new session.`)
            process.exit(-1);
        }
        const request = {
            "type": site ? "get_specific_sessions" : "get_sessions",
            "experiment": ZMQ_EXPERIMENT,
            ...((site && { site }) || { k: count })
        }
        this.state = "sessions_request";

        Logging.info("Requesting ZMQ sessions")
        // Request the session via ZMQ
        await this.sock.send(JSON.stringify(request))
        // Parse the result
        const [result] = await this.sock.receive()
        ...
        if (parsedResult.success) {
            ...
            try {
                // Create session in databse if it does not exist
                const { site, sessions } = parsedResult;

                const [sessionGroup] = await SessionGroup.findOrCreate({
                    where: {
                        site: site
                    },
                    transaction: t
                ...
                for (var { session, session_data } of sessions) {
                    const { id } = session;
            ...
            // If success was false, return/do nothing and output log message
            Logging.error(`Session request did not yield new session. success=false, error=${parsedResult.error}`)
        }
    }
...
class ZMQWrapperTest {
    ...
    async getSessions(site?: string, count = 2): Promise<Session[]> {
